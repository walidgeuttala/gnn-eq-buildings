...
def objective(params):
    ...
    pre_lstm_model.fit(x_train,y_train,epochs=10,batch_size=100,verbose=0)

    lstm_pred = pre_lstm_model.predict(x_test).reshape(-1,1)
    lstm_pred = t_y_slr.inverse_transform(lstm_pred)

    base_r2 =  r2_score(y_test,lstm_pred)
    base_rmse = return_rmse(y_test,lstm_pred)
    ...
    new_lstm_model.fit(x_train,y_train,epochs=10,batch_size=10,verbose=0)

    lstm_pred2 = new_lstm_model.predict(x_test).reshape(-1,1)
    lstm_pred2 = t_y_slr.inverse_transform(lstm_pred2)

    TL_r2 =  r2_score(y_test,lstm_pred2)
    TL_rmse = return_rmse(y_test,lstm_pred2)
...
space = {
    'lookback': hp.choice('lookback', [5,10,25,50,100,200]),
    '1st': hp.choice('1st', [10,25,50,100,200]),
    '2nd': hp.choice('2nd', [10,25,50,100,200]),
    'fnn': hp.choice('fnn', [5,10,20,30]),
    }
